...
class ts2music:
    ...
    def adjust_key_to_scale(self, n):
        ...
        pianokeys = np.array([])
        for i in range(7):
            pianokeys = np.append(pianokeys,
                                  self.fundamental + self.scale + 12*i)
            # print(pianokeys)
    ...
    def add_notes_to_timeseries(self):
        ...
        cases_qnt = self.timeseries[self.col][
            self.timeseries[self.col] > 0].quantile([.05, .95])
        lowcases = cases_qnt[0.05]
        highcases = cases_qnt[0.95]
        caseskeys = np.repeat(0, self.timeseries.shape[0])

        # low numbers of cases
        ind = np.where((self.timeseries[self.col] > 0) &
                       (self.timeseries[self.col] <= lowcases))[0]
        caseskeys[ind] = ((self.timeseries[self.col][ind] / lowcases) *
                          (self.midkey - self.lowkey) + self.lowkey)

        # mid numbers of cases
        ind = np.where((self.timeseries[self.col] > 0) &
                       (self.timeseries[self.col] > lowcases) &
                       (self.timeseries[self.col] <= highcases))[0]

        caseskeys[ind] = np.floor(
            ((self.timeseries[self.col][ind] - lowcases) /
             (highcases - lowcases)) *
        ...
        ind = np.where((self.timeseries[self.col] > 0) &
                       (self.timeseries[self.col] > highcases))[0]
        caseskeys[ind] = np.floor(
            ((self.timeseries[self.col][ind] - highcases) /
             (maxcases - highcases)) *
            (self.maxkeys - self.highkey) + self.highkey
        )

        self.timeseries['Notes'] = np.array(
            [self.adjust_key_to_scale(n) for n in caseskeys]
        )
...
def standarize_timeseries(list_of_ts, names_in, unique_time):
    ...
    for nn in range(len(sorted_year)):
        for ff in range(len(list_of_ts)):
            ...
            df = df_tmp[(df_tmp['Year'] == sorted_year[nn]) &
                        (df_tmp['Week'] == sorted_weeks[nn])]
            if not df.empty:
...
def plot_timeseries(df, day, names_in, dir_figs):
    ...
    for x in range(len(axarr)):
        ...
        axarr[x].plot(time_ts, df[names_in[x]],
                      linestyle='-', color='black',
                      alpha=1.0, linewidth=2)
        axarr[x].plot([time_ts[day],
                       time_ts[day]],
                      [0, maxcases], color='gray')
...
def main():
    ...
    parser.add_argument("-f", type=str,
                        help="Select the file with the timeseries",
                        required=True)
    parser.add_argument("-s", type=str,
                        help="Scale in international notation",
                        default='C')
    parser.add_argument("-o", type=str,
                        help="Video output",
                        default='output')
    parser.add_argument("-t", type=int,
                        help="Tempo [default 180]",
                        default=180)
    parser.add_argument("--minor",
                        help="Enable minor scale",
    ...
    time_tuple = list()
    # [41,1,74]
    midi_programs = list(filenames['MidiInstrument'])
    for ff in range(filenames.shape[0]):
        ts_tmp = pd.read_csv(filenames['File'][ff],
                             sep='\s*,\s*',
                             engine='python')
        time_tuple.extend([(ts_tmp['Year'][n],
                            ts_tmp['Week'][n])
                           for n in range(ts_tmp.shape[0])])
        ts_tmp['Year.Week'] = ts_tmp['Year']+ts_tmp['Week']/52.0
        timeseries_list.append(ts_tmp)

    std_case_data = standarize_timeseries(
        timeseries_list, names_list, set(time_tuple))
    for n in range(len(names_list)):
        ...
        ptmp.insert(0, mus.tempo.MetronomeMark(
            number=tempo,
            referent=mus.note.Note(type='quarter')))
